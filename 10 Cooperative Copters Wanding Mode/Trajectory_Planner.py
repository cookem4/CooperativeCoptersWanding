...
class Trajectory_Planner:
        ...
        ##################################################
        ####### Set this from CSV File ###################
        ##################################################
        #setPointCoordinates = [[0,0,0,0], [0,0,1.1,6], [0,0,1.1,10], [0,0,0,6]] # The sequence of coordinates for the leader copter to get to at a certain time, [x,y,z, Time] all in the world frame!
        setPointCoordinates = []                                                                # NOTE: the initial coordinate of the leader is 0 0 0.
        coffsX = []
        coffsY = []
        coffsZ = []
        #xOffsets = [0] # X coordinates of all copters w.r.t the leader [0, 1.2, 0.5]...........marked on mats:[0,1.5,0.82]
        #Uncomment for two drones
        xOffsets = [0,0]
        yOffsets = [0,-1.6] # Y coordinates of all copters w.r.t the leader   [0, 0, -0.8]..........marked on mats:[0,0,-0.78]
        #yOffsets = [0]
        ...
        def extract_traj(self):
                trajectories = []
                with open('C:\\Users\\cookem4\\Desktop\\Spiral.csv') as csv_file:
                        csv_reader = csv.reader(csv_file, delimiter = ',')
                        line_count = 0
                        initialPos = []
                        #XYZ from the motiv system needs to be mapped to the flight controller
                        for row in csv_reader:
                                if(line_count == 0):
                                        initialPos = [float(row[6]), -1*float(row[8]), float(row[7])]
                                        trajectories.append([float(row[6])-initialPos[0], -1*float(row[8]) - initialPos[1], float(row[7])-initialPos[2], 0])
                                elif(line_count%12==0):
                                        #Time is hardcoded to 0.1 for 10Hz
                                        trajectories.append([float(row[6])-initialPos[0], -1*float(row[8]) - initialPos[1], float(row[7])-initialPos[2], 0.1])
                                line_count+=1
                self.setPointCoordinates = trajectories
